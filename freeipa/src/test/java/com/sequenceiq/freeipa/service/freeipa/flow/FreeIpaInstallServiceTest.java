...
class FreeIpaInstallServiceTest {
    ...
    public void testInstall() throws CloudbreakOrchestratorException {
        ...
        Set<String> recipeNames = Set.of("recipe1", "recipe2");
        when(stack.getRecipes()).thenReturn(recipeNames);
        ...
        when(freeIpaRecipeService.getRecipes(recipeNames)).thenReturn(recipeModelList);
        ...
        verify(hostOrchestrator).postClusterManagerStartRecipes(eq(gatewayConfig), eq(nodes), any(StackBasedExitCriteriaModel.class));
    }
