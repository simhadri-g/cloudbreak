...
public class FreeIpaInstallService {
    ...
    private void installFreeIpa(Long stackId, Stack stack, List<GatewayConfig> gatewayConfigs, Set<Node> allNodes) throws CloudbreakOrchestratorException {
        ...
        List<RecipeModel> recipes = freeIpaRecipeService.getRecipes(stack.getRecipes());
        LOGGER.info("Recipes for stack: {}", recipes);
        hostOrchestrator.uploadRecipes(gatewayConfigs, Map.of("master", recipes), new StackBasedExitCriteriaModel(stackId));
        ...
        hostOrchestrator.postClusterManagerStartRecipes(primaryGatewayConfig, allNodes, new StackBasedExitCriteriaModel(stackId));
    }
}